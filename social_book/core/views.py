...
def index(request):
    ...
    final_suggertions_list=[x for x in list(new_suggestions_list) if (x not in list(current_user))]

    random.shuffle(final_suggertions_list)

    username_profile = []
    username_profile_list =[]

    for users in final_suggertions_list:
    ...
    if request.method == "POST":
        ...
        if password==password2:
            if User.objects.filter(email=email).exists():
                ...
                # lets create profile model for the new user
...
def signin(request):
    if request.method=="POST":
        ...
        if user is not None:
            ...
            return redirect('/')
        elif auth.authenticate(username=username) is not None:
